...
function run = COPSE_frontend(S,ffeedback,pfeedback)
...
%     pars.plantenhance = 0.15 ;
   ...
   % if ffeedback == "vitali" || pfeedback == "vitali" || ffeedback == "combined" || pfeedback == "combined" 
       ...
       feedbackd = importfile("/Volumes/PhD stuff/Models/COPSE-main/new_forcings/bgspreadsheet_updated.xlsx");
        ...
        forcings.bothox = feedbackd.bothBionorm;      %%% combined effect of fire and productivity from DGVM data
   % end
...
%     outputs = [ 0.5 1.2 2.5 1 0.1 1 3] ;
    ...
    %%%%%%% run the system 
   [rawoutput.T,rawoutput.Y] = ode15s(@(t,y) COPSE_equations(t,y,ffeedback,pfeedback),[pars.whenstart pars.whenend],pars.startstate,options);
