...
%%%% effect of O2 on VEG
if pfeedback == "none" || pfeedback == "combined" || ffeedback == "combined"
                                                  % no productivity feedback also feedback including interaction
    V_o2 = 1;                                     % between fire and producivity to avoid double effect 
    
elseif pfeedback == "vitali"                      % feedback based on DGVM runs (updated)
    V_o2 = interp1qr(forcings.oxygen/20.95, forcings.prodox,(O/pars.O0)); % interpolate for oxygen concentration
    
else                                              % standard baseline model feedback
    V_o2 = 1.5 - 0.5*(O/pars.O0) ;                %% COPSE forumla
end


%%%% calculate O2 compensation point following andre(2011)
ocomp = cpoint(Tsurf-273.15, CO2atm*1e6);
...
%%% fire feedback
if ffeedback == "none"                            % no fire feedback
    firef = 1;
    
elseif ffeedback == "vitali"                      % feedback based on DGVM runs 
    firef = interp1qr(forcings.oxygen, forcings.fireox,mrO2*100); % interpolate for given oxygen at this timestep
    
elseif ffeedback == "combined" || pfeedback == "combined"
    firef = interp1qr(forcings.oxygen,forcings.bothox,mrO2*100); % interpolate for given oxygen at this timestep
    
else                                              % COPSE reloaded standard fire feedback 
    k_f = pars.kfire;                             % bond et al. suppression
    ignit = min(max(48*mrO2 - 9.08 , 0) , 5 ) ;
    % ignit = max(586.2*(mrO2)-122.102 , 0  ) ;   % old original COPSE
    firef = k_f/(k_f - 1 + ignit) ;
end
...
if sensanal == 0
    ...
    %workingstate.k_f(stepnumber,1) = k_f;
    workingstate.firef(stepnumber,1) = firef;
    workingstate.cpoint(stepnumber,1) = ocomp;
    workingstate.V_o2(stepnumber,1) = V_o2; % oxygen suppression on productivity
